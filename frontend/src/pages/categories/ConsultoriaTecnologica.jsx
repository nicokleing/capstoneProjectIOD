...
const ConsultoriaTecnologica = () => {
  ...
  useEffect(() => {
    const fetchServices = async () => {
      try {
        const { data } = await axios.get('/api/services?category=Tech Consulting Services', {
          headers: {
        ...
        console.error('Error fetching tech consulting services:', error);
        setError('Error fetching tech consulting services');
        setLoading(false);
      }
    };
  ...
  return (
    <Container>
      <h1>Tech Consulting Services</h1>

      {/* Section for Video */}
      <div className="video-section my-4">
        <video
          className="d-block w-100"
          src="https://videos.pexels.com/video-files/2795165/2795165-uhd_2560_1440_25fps.mp4"
          autoPlay
          loop
          muted
        />
